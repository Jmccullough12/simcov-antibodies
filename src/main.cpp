...
void update_virions(GridPoint *grid_point, vector<int64_t> &nbs,
                    HASH_TABLE<int64_t, float> &virions_to_update, double ab_conc) {
  update_concentration_timer.start();
  if (_options->use_antibodies) {
    double clearance_rate = _options->virion_clearance_rate + (ab_conc * _options->antibody_effect);

    //grid_point->virions = grid_point->virions * (1.0 - _options->virion_clearance_rate);
    //grid_point->virions = grid_point->virions * (1.0 - (_options->virion_clearance_rate + ((ab_conc * 1.0) * .003)));
    _options->current_clearance = clearance_rate;
    grid_point->virions = grid_point->virions * (1.0 - _options->current_clearance);
    //grid_point->virions = grid_point->virions * (1.0 - clearance_rate);
  }
  else {
    grid_point->virions = grid_point->virions * (1.0 - _options->virion_clearance_rate);
  }
...
void run_sim(Tissue &tissue) {
  ...
  for (int time_step = 0; time_step < _options->num_timesteps; time_step++) {
    ...
    for (auto grid_point = tissue.get_first_active_grid_point(); grid_point;
      ...
      update_virions(grid_point, *nbs, virions_to_update, ab_conc);
