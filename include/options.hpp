...
class Options {
 ...
 public:
  ...
  // New Options for command line access
  ...
  bool load(int argc, char **argv) {
    ...
    // NEW OPTIONS
    app.add_option("--use_antibodies", use_antibodies,
                   "Toggle for using new implementation of antibodies")
        ->capture_default_str();
    app.add_option("--antibody_max", antibody_max,
                   "Max Load of antobodies (in titres)")
        ->capture_default_str();
    app.add_option("--antibody_effect", antibody_effect,
                   "Fraction by which virion count drops each time step at max ab load")
        ->capture_default_str();
    app.add_option("--antibody_scale", antibody_scale,
                   "Scale applied to antibody_effect")
        ->capture_default_str();
